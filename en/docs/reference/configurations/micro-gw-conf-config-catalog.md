...
|`keyManager`|The Key Manager related information. This information is required when the Microgateway connects with the Key Manager in order to validate the tokens. (for configurations, please check [the documentation on Securing APIs Using Opaque Tokens]({{base_path}}/how-tos/security/api-authentication/secure-apis-using-oauth2.0-access-tokens/secure-apis-using-opaque-tokens/) )|`serverUrl`|Connection URL of the Key Manager server. By default, this is WSO2 Identity Server (WSO2 IS).|<https://localhost:9443>|
