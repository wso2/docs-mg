...
Follow below steps to enable Real-time and Persistent token revocation. This documentation explains steps required to handle revoked JWTs. Same steps can be used for handling revoked opaque tokens. However for opaque tokens, you only need to enable realtime notifications.
...
1. Navigate to the `<MGW_HOME>/conf/micro-gw.conf` file and add the following configuration.
    ...
    !!! note
        When working with persistent notifications, if you introduced new configurations in your custom implementation, then you have to provide the corresponding values for those propertyies in the `<MGW_HOME>/conf/micro-gw.conf` file under the `[tokenRevocationConfig.persistent]` section.
